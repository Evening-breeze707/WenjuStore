...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 7% 0","alignItems":"center","background":"none","display":"flex","width":"100%","justifyContent":"center","height":"auto"}'>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","fontSize":"16px","display":"inline-block"}'>文具名称</div>
        <el-input v-model="formSearch.wenjumingcheng" placeholder="文具名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","fontSize":"16px","display":"inline-block"}'>文具品牌</div>
        <el-input v-model="formSearch.wenjupinpai" placeholder="文具品牌" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","fontSize":"16px","display":"inline-block"}'>价格</div>
	  ...
	  <el-button v-if=" true " :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#cca162","width":"auto","fontSize":"16px","lineHeight":"42px","height":"42px"}' type="primary" @click="getList(1, curFenlei)"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"16px"}' class="el-icon-search"></i>查询</el-button>
	...
	<div class="list" :style='{"padding":"120px 7% 100px","margin":"20px 0 10px","background":"url(http://codegen.caihongy.cn/20230109/138a4d606eae4a168810cef229969885.png) no-repeat center top,url(http://codegen.caihongy.cn/20230109/b95aeae1c1294f398aefb7c6cd5b16de.png) no-repeat center top,url(http://codegen.caihongy.cn/20230109/5ffc803e6682418eb7f0b09a98e35527.png) no-repeat center bottom,#faf0e6"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0px","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"1px solid #ddd","padding":"10px","margin":"0 0 20px","flexWrap":"wrap","background":"#fff","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"100%","margin":"0 0 8px","objectFit":"contain","display":"block","height":"275px"}' v-if="item.wenjutupian && item.wenjutupian.substr(0,4)=='http'" :src="item.wenjutupian" class="image" />
				<img :style='{"width":"100%","margin":"0 0 8px","objectFit":"contain","display":"block","height":"275px"}' v-else :src="baseUrl + (item.wenjutupian?item.wenjutupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"border":"1px solid #eee","padding":"4px 10px","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"20px","width":"100%","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis"}' class="name ">{{item.wenjumingcheng}}</div>
				<div :style='{"border":"1px solid #eee","padding":"4px 10px","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"20px","width":"100%","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis"}' class="name ">{{item.wenjufenlei}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
        this.$http.get('option/wenjufenlei/wenjufenlei').then(res => {
          if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.wenjumingcheng != '') searchWhere.wenjumingcheng = '%' + this.formSearch.wenjumingcheng + '%';
        if (this.formSearch.wenjupinpai != '') searchWhere.wenjupinpai = '%' + this.formSearch.wenjupinpai + '%';
        ...
        if (this.curFenlei != '全部') searchWhere.wenjufenlei = this.curFenlei;
        this.$http.get('wenjuxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/wenjuxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
